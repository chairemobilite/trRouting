...
namespace TrRouting
{
  ...
  void Calculator::prepare()
  {
    ...
    if (params.dataFetcherShortname == "database")
    {
      ...
      {
        std::tie(odTrips, odTripIndexesById, odTripFootpaths) = params.databaseFetcher->getOdTrips(params.applicationShortname, stops, params);
      }
      else
      {
    ...
    stopsReverseTentativeTime              = std::vector<int>(stops.size());
    ...
    forwardJourneys                        = std::vector<std::tuple<int,int,long long,int,int,short>>(stops.size());
    forwardEgressJourneys                  = std::vector<std::tuple<int,int,long long,int,int,short>>(stops.size());
    reverseJourneys                        = std::vector<std::tuple<int,int,long long,int,int,short>>(stops.size());
    reverseAccessJourneys                  = std::vector<std::tuple<int,int,long long,int,int,short>>(stops.size());
