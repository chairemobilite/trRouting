...
namespace TrRouting
{
  ...
  private:
    ...
    std::vector<std::pair<long long,long long>> footpathsRanges; // index: stopIndex, pair: index of first footpath, index of last footpath
    ...
    std::vector<std::pair<int,int>>       odTripFootpaths; // pair: stopIndex, walkingTravelTimeSeconds
    std::vector<std::tuple<int,int,long long,int,int,short>> forwardJourneys; // index = stop index, tuple: final enter connection, final exit connection, final footpath, final exit trip index, transfer travel time, is same stop transfer (first, second, third and fourth values = -1 for access and egress journeys)
    std::vector<std::tuple<int,int,long long,int,int,short>> forwardEgressJourneys; // index = stop index, tuple: final enter connection, final exit connection, final footpath, final exit trip index, transfer travel time, is same stop transfer (first, second, third and fourth values = -1 for access and egress journeys)
    std::vector<std::tuple<int,int,long long,int,int,short>> reverseJourneys; // index = stop index, tuple: final enter connection, final exit connection, final footpath, final exit trip index, transfer travel time, is same stop transfer (first, second, third and fourth values = -1 for access and egress journeys)
    std::vector<std::tuple<int,int,long long,int,int,short>> reverseAccessJourneys; // index = stop index, tuple: final enter connection, final exit connection, final footpath, final exit trip index, transfer travel time, is same stop transfer (first, second, third and fourth values = -1 for access and egress journeys)
    int                                   maxTimeValue;
