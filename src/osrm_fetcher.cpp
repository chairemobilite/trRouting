...
namespace TrRouting
{
  std::vector<std::tuple<int,int,int>> OsrmFetcher::getAccessibleNodesFootpathsFromPoint(const Point point, const std::vector<std::unique_ptr<Node>>& nodes, std::string mode, Parameters& params, bool reversed)
  {
    ...
    {
      return getNodesFromOsmr(point, nodes, mode, params, reversed);
    }
  }
  ...
  std::vector<std::tuple<int,int,int>> OsrmFetcher::getNodesFromOsmr(const Point& point, const std::vector<std::unique_ptr<Node>>& nodes, std::string mode, Parameters& params, bool reversed)
  { 
